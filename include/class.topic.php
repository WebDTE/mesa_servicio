...
implements TemplateVariable, Searchable {
    ...
    static function getHelpTopics($mod=false,$publicOnly=false, $disabled=false, $localize=true, $whitelist=array()) {
      global $cfg;
      ...
      if (!$names || $localize) {
          ...
          if($mod){
             ...
             'topic_id', 'topic_pid', 'ispublic', 'flags', 'topic')->
                  filter(array('topic_pid' => $mod))->
                  order_by('sort');
          }else{
               $objects = self::objects()->values_flat('topic_id', 'topic_pid', 'ispublic', 'flags', 'topic')->
                  order_by('sort');
          }
    ...
    static function getHelpTopicsByParent($topic_pid) {
        return self::getHelpTopics($topic_pid,true);
    }
