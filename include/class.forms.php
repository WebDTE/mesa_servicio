...
class FileUploadField extends FormField {
    ...
    function ajaxUpload($bypass=false) {
        ...
        if (count($files) != 1)
            Http::response(400, 'Envía un archivo a la vez');
        $file = array_shift($files);
        $file['name'] = urldecode($file['name']);

        if (!$this->isValidFile($file))
            Http::response(413, 'Archivo no valido');

        if (!$bypass && !$this->isValidFileType($file['name'], $file['type']))
            Http::response(415, 'Tipo de archivo no permitido');

        $config = $this->getConfiguration();
        if (!$bypass && $file['size'] > $config['size'])
            Http::response(413, 'El archivo pesa mas MB que los permitidos');

        if (!($F = AttachmentFile::upload($file)))
            Http::response(500, 'Error al guardar el archivo: '. $file['error']);
